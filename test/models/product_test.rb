...
class ProductTest < ActiveSupport::TestCase
  ...
  test 'should sort product by most recent' do
    ...
    assert_equal [
      ...
      products(:one),
      products(:two)
    ], Product.order_by_updated_at.to_a
  end
  ...
  test 'search should get recent products' do
    search_hash = { recent: true }
    products(:two).touch
    assert_equal products(:two), Product.search(search_hash).last
  end
end
